...
protected:
    ...
    std::vector<uint8_t> createUniformImage(uint32_t width, uint32_t height, int32_t channels, uint8_t value) {
    ...
    std::vector<uint8_t> createHighValueImage(uint32_t width, uint32_t height, int32_t channels, uint8_t value = 255) {
        std::vector<uint8_t> image(width * height * channels, value);
        return image;
    }
};
...
TEST_F(DownscaleAreaAverageTest, BasicDownscaling) {
    ...
    downscale_area_average(src.data(), 4, 4, dst.data(), 2, 2, 3);
...
TEST_F(DownscaleAreaAverageTest, MathematicalCorrectness) {
    ...
    downscale_area_average(src.data(), 2, 2, dst.data(), 1, 1, 1);
...
TEST_F(DownscaleAreaAverageTest, SingleChannel) {
    auto src = createTestImage(4, 4, 1);
    std::vector<uint8_t> dst(2 * 2 * 1);
    
    downscale_area_average(src.data(), 4, 4, dst.data(), 2, 2, 1);
...
TEST_F(DownscaleAreaAverageTest, FourChannels) {
    auto src = createTestImage(4, 4, 4);
    std::vector<uint8_t> dst(2 * 2 * 4);
    
    downscale_area_average(src.data(), 4, 4, dst.data(), 2, 2, 4);
...
TEST_F(DownscaleAreaAverageTest, ExtremeDownscaling) {
    ...
    downscale_area_average(src.data(), 8, 8, dst.data(), 1, 1, 3);
...
TEST_F(DownscaleAreaAverageTest, NonIntegerScaling) {
    ...
    downscale_area_average(src.data(), 3, 3, dst.data(), 2, 2, 3);
...
TEST_F(DownscaleAreaAverageTest, RectangularImages) {
    ...
    downscale_area_average(src.data(), 4, 2, dst.data(), 2, 1, 3);
...
TEST_F(DownscaleAreaAverageTest, BoundaryConditions) {
    ...
    downscale_area_average(src.data(), 5, 5, dst.data(), 3, 3, 3);
...
TEST_F(DownscaleAreaAverageTest, LargeImageOverflowTest) {
    ...
    downscale_area_average(src.data(), 256, 256, dst.data(), 1, 1, 1);
...
TEST_F(DownscaleAreaAverageTest, LargeImageOverflowEdgeCase) {
    ...
    downscale_area_average(src.data(), 512, 512, dst.data(), 1, 1, 1);
